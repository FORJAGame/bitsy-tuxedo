...
Todo sala tem uma **paleta**: uma pequena lista de cores de onde toda a arte daquele sala é feita. Salas podem compartilhar uma paleta ou cada um usar a sua própria.
...
Paletas podem ser renomeadas (o nome aparece na ferramenta [Find](find.md) e nas configurações do sala), e você pode criar, duplicar, navegar entre, ou deletar paletas com os controles do painel.
...
Isso é útil pra desenhos que querem mais do que um visual simples de dois tons (por exemplo, um sprite com uma cor de destaque, ou um sala com um gradiente de fundo mais rico) sem precisar reaproveitar os papéis de tile/sprite pra tudo.
